                    ...
                    <form @submit.prevent="register" class="space-y-4 md:space-y-6">
                        ...
                        <div class="flex items-start">
                            <div class="flex items-center h-5">
                                <input id="terms" aria-describedby="terms" type="checkbox" class="w-4 h-4 border border-gray-300 rounded bg-gray-50 focus:ring-3">
                            </div>
                        ...
                        <button type="submit" class="w-full text-white bg-blue-600 hover:bg-primary-700 focus:ring-4 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Create an Account</button>
                        <p class="text-sm font-light text-gray-500 dark:text-gray-400">
                            Already have an account? <router-link to="/login" class="font-medium text-primary-600 hover:underline">Login here</router-link>
                        </p>
                    </form>
                </div>
            </div>
...
<script setup lang="ts">
    ...
    const register = async () => {
        try {
            ...
            const response = await store.dispatch('register/signup', payload);
            if (response.status === 200) {
                ToastifySuccess('User create successful');
                email.value = '',
                password.value = ''
            }
        } catch (err) {
            console.error(err);
            ToastifyError('User create fail, Pleace try again');
        }
    }
</script>